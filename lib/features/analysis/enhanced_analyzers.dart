...
class ConversationDynamicsAnalyzer {
  static const int conversationGapMinutes =
      30; // Gap that defines new conversation
  static const int rapidFireSeconds = 10; // Quick response threshold

  List<List<MessageEntity>> _groupMessagesIntoConversations(
    ...
    const conversationGapMinutes = 60; // 1 hour gap = new conversation
...
// ADD this method to ConversationDynamicsAnalyzer class
  ...
  Future<Map<String, dynamic>> analyze(ChatEntity chat) async {
    try {
      debugPrint("ConversationDynamicsAnalyzer: Starting analysis");

      // For large chats, limit processing to avoid timeouts
      final messagesToProcess = chat.messages.length > 10000
          ? chat.messages.take(10000).toList()
          : chat.messages;

      final conversations = _groupMessagesIntoConversations(messagesToProcess);
      debugPrint(
          "ConversationDynamicsAnalyzer: Found ${conversations.length} conversations");
      ...
      final initiationPatterns =
          _analyzeInitiationPatterns(conversations, chat.users);

      // Calculate health score (simplified for performance)
      final healthScore = _calculateHealthScore(conversations, userIdToName);
      ...
      return {
        'conversationDynamics': {
          ...
          'avgConversationLength': conversations.isNotEmpty
              ? (conversations.map((c) => c.length).reduce((a, b) => a + b) /
  ...
  int _calculateHealthScore(List<List<MessageEntity>> conversations,
      Map<String, String> userIdToName) {
    if (conversations.isEmpty) return 0;

    try {
      // Simple health calculation based on conversation balance
      final userParticipation = <String, int>{};

      for (final conversation in conversations.take(100)) {
        // Limit for performance
        for (final message in conversation) {
          final userName = userIdToName[message.senderId] ?? 'Unknown';
          userParticipation[userName] = (userParticipation[userName] ?? 0) + 1;
        }
      }

      if (userParticipation.isEmpty) return 0;

      // Calculate balance (higher score for more balanced conversations)
      final values = userParticipation.values.toList();
      final maxMessages = values.reduce((a, b) => a > b ? a : b);
      final minMessages = values.reduce((a, b) => a < b ? a : b);

      final balance = minMessages / maxMessages;
      return (balance * 100).round().clamp(0, 100);
    } catch (e) {
  ...
  List<List<MessageEntity>> _identifyConversations(
      List<MessageEntity> messages) {
    final conversations = <List<MessageEntity>>[];
    List<MessageEntity> currentConversation = [];
  ...
  Map<String, dynamic> _analyzeConversationStats(
      List<List<MessageEntity>> conversations) {
    if (conversations.isEmpty)
      return {'avgLength': 0, 'longest': 0, 'shortest': 0};
  ...
  Map<String, dynamic> _analyzeDialogFlows(
      List<List<MessageEntity>> conversations) {
    ...
    for (final sender in senders) {
      if (sender == lastSender) {
        ...
        if (consecutiveCount > 0) {
          pattern
              .write(consecutiveCount > 2 ? 'M' : 'S'); // M=Multiple, S=Single
        }
        consecutiveCount = 1;
        lastSender = sender;
      }
  ...
  Map<String, dynamic> _analyzeRapidFireExchanges(
      List<MessageEntity> messages) {
    ...
    for (int i = 1; i < messages.length; i++) {
      final gap =
          messages[i].timestamp.difference(messages[i - 1].timestamp).inSeconds;
      if (gap <= rapidFireSeconds &&
    ...
    return {
      ...
      'rapidFirePercentage': messages.isEmpty
          ? 0
...
class BehaviorPatternAnalyzer {
  Future<Map<String, dynamic>> analyze(ChatEntity chat) async {
    try {
      ...
      final messagesToProcess = chat.messages.length > 8000
          ? chat.messages.take(8000).toList()
          : chat.messages;
      ...
      final compatibilityScore =
          _calculateCompatibilityScore(messagesToProcess, userIdToName);

      // Communication styles (simplified)
      final communicationStyles =
          _analyzeCommunicationStyles(messagesToProcess, userIdToName);
  ...
  Map<String, dynamic> _analyzeCommunicationStyles(
    ...
    try {
      ...
      // Analyze sample of messages for performance
      for (final message in messages.take(2000)) {
        final userName = userIdToName[message.senderId] ?? 'Unknown';

        if (userStats.containsKey(userName)) {
          userStats[userName]!['totalMessages'] =
              (userStats[userName]!['totalMessages'] as int) + 1;
          userStats[userName]!['totalWords'] =
              (userStats[userName]!['totalWords'] as int) +
                  message.content.split(' ').length;

          // Count emojis
          final emojiRegex = RegExp(r'[\u{1F600}-\u{1F64F}]', unicode: true);
          final emojiCount = emojiRegex.allMatches(message.content).length;
          userStats[userName]!['emojiCount'] =
              (userStats[userName]!['emojiCount'] as int) + emojiCount;
        }
      }

      // Calculate averages and determine styles
      for (final entry in userStats.entries) {
        final stats = entry.value;
        final totalMessages = stats['totalMessages'] as int;

        if (totalMessages > 0) {
          final avgLength = (stats['totalWords'] as int) / totalMessages;
          stats['avgMessageLength'] = avgLength;

          // Simple style classification
          if (avgLength > 15) {
            stats['styleType'] = 'Detailed Communicator';
          } else if (avgLength < 5) {
            stats['styleType'] = 'Concise Communicator';
          } else {
            stats['styleType'] = 'Balanced Communicator';
          }
        }

        styles[entry.key] = stats;
      }
    } catch (e) {
      debugPrint("Error analyzing communication styles: $e");
    }
  ...
  Map<String, dynamic> _analyzeTimePersonalities(
    ...
    for (final entry in userHours.entries) {
      ...
      personalities[userName] = {
        ...
        'nightPercentage':
            ((nightMessages / totalMessages) * 100).toStringAsFixed(1),
        'morningPercentage':
            ((morningMessages / totalMessages) * 100).toStringAsFixed(1),
        'afternoonPercentage':
            ((afternoonMessages / totalMessages) * 100).toStringAsFixed(1),
        'eveningPercentage':
            ((eveningMessages / totalMessages) * 100).toStringAsFixed(1),
  ...
  Map<String, dynamic> _analyzeConsistency(
    ...
    for (final message in messages) {
      final userName = userIdToName[message.senderId] ?? 'Unknown';
      final date = DateTime(message.timestamp.year, message.timestamp.month,
    ...
    for (final entry in userDates.entries) {
      ...
      final variance =
          gaps.map((g) => math.pow(g - avgGap, 2)).reduce((a, b) => a + b) /
              gaps.length;
      final consistencyScore =
          math.max(0, 100 - (variance * 2)); // Scale to 0-100
  ...
  Map<String, dynamic> _analyzeWeekendBehavior(
    ...
    for (final message in messages) {
      final userName = userIdToName[message.senderId] ?? 'Unknown';
      final isWeekend =
          message.timestamp.weekday >= 6; // Saturday = 6, Sunday = 7

      userDayStats.putIfAbsent(userName, () => {'weekday': 0, 'weekend': 0});

      if (isWeekend) {
        userDayStats[userName]!['weekend'] =
            userDayStats[userName]!['weekend']! + 1;
      } else {
        userDayStats[userName]!['weekday'] =
            userDayStats[userName]!['weekday']! + 1;
      }
    }
  ...
  Map<String, dynamic> _analyzeEnergyLevels(
    ...
    for (final entry in userMessageData.entries) {
      ...
      final avgLength =
          messageData.map((m) => m['length'] as int).reduce((a, b) => a + b) /
              messageData.length;
      final avgExclamations = messageData
              .map((m) => m['exclamations'] as int)
              .reduce((a, b) => a + b) /
          messageData.length;
      final avgCaps =
          messageData.map((m) => m['caps'] as int).reduce((a, b) => a + b) /
              messageData.length;
      final avgQuestions = messageData
              .map((m) => m['questions'] as int)
              .reduce((a, b) => a + b) /
          messageData.length;
  ...
  Map<String, dynamic> _analyzeSeasonalPatterns(
    ...
    if (daySpan < 90) {
      return {
        'message': 'Not enough data for seasonal analysis (need 90+ days)'
      };
    }
    ...
    for (final message in messages) {
      ...
      userMonthStats[userName]![month] =
          (userMonthStats[userName]![month] ?? 0) + 1;
    }
    ...
    for (final entry in userMonthStats.entries) {
      ...
      seasonalPatterns[userName] = {
        ...
        'monthlyStats': {
          for (var e in monthStats.entries) monthNames[e.key]: e.value
        },
      };
    }
  ...
  Map<String, dynamic> _analyzePunctuationPersonality(
    ...
    for (final message in messages) {
      ...
      userPunctuation[userName]!['periods'] =
          userPunctuation[userName]!['periods']! +
              (content.split('.').length - 1);
      userPunctuation[userName]!['exclamations'] =
          userPunctuation[userName]!['exclamations']! +
              (content.split('!').length - 1);
      userPunctuation[userName]!['questions'] =
          userPunctuation[userName]!['questions']! +
              (content.split('?').length - 1);
      userPunctuation[userName]!['ellipsis'] =
          userPunctuation[userName]!['ellipsis']! +
              content.split('...').length -
              1;
      userPunctuation[userName]!['totalMessages'] =
          userPunctuation[userName]!['totalMessages']! + 1;
    }
  ...
  int _calculateCompatibilityScore(
    ...
    try {
      ...
      // Simple compatibility based on response times and message frequency
      final userMessageCounts = <String, int>{};

      for (final message in messages.take(1000)) {
        // Limit for performance
        final userName = userIdToName[message.senderId] ?? 'Unknown';
        userMessageCounts[userName] = (userMessageCounts[userName] ?? 0) + 1;
      }

      if (userMessageCounts.length < 2) return 0;
...
class RelationshipAnalyzer {
  Future<Map<String, dynamic>> analyze(ChatEntity chat) async {
    try {
      ...
      // Limit messages for performance
      final messagesToProcess = chat.messages.length > 6000
          ? chat.messages.take(6000).toList()
          : chat.messages;

      final userIdToName = {for (var user in chat.users) user.id: user.name};

      // Simple relationship health calculation
      final healthScore =
          _calculateRelationshipHealthScore(messagesToProcess, userIdToName);

      // Support patterns (simplified)
      final supportPatterns =
          _analyzeSupportPatterns(messagesToProcess, userIdToName);
  ...
  int _calculateRelationshipHealthScore(
      List<MessageEntity> messages, Map<String, String> userIdToName) {
    try {
      if (messages.length < 50) return 0;

      // Simple health score based on mutual engagement
  ...
  Map<String, dynamic> _analyzeSupportPatterns(
    ...
    try {
      final userSupport = <String, Map<String, int>>{};
      ...
      final helpWords = ['help', 'support', 'assist', 'advice', 'suggest'];
      ...
      for (final message in messages.take(1500)) {
        // Limit for performance
        final userName = userIdToName[message.senderId] ?? 'Unknown';
        ...
        if (userSupport.containsKey(userName)) {
          ...
          if (questionWords.any((word) => content.contains(word))) {
            userSupport[userName]!['questionsAsked'] =
                userSupport[userName]!['questionsAsked']! + 1;
          }

          // Count help offerings
          if (helpWords.any((word) => content.contains(word))) {
            userSupport[userName]!['helpProvided'] =
                userSupport[userName]!['helpProvided']! + 1;
          }

          // Count emotional support
          if (emotionalWords.any((word) => content.contains(word))) {
            userSupport[userName]!['emotionalSupport'] =
                userSupport[userName]!['emotionalSupport']! + 1;
          }
        }
      }

      // Add support type classification
      for (final entry in userSupport.entries) {
        final stats = entry.value;
        final total = stats.values.reduce((a, b) => a + b);

        String supportType = 'Balanced Supporter';
        if (stats['questionsAsked']! > stats['helpProvided']!) {
          supportType = 'Question Asker';
        } else if (stats['helpProvided']! > stats['questionsAsked']!) {
          supportType = 'Helper';
        } else if (stats['emotionalSupport']! > total * 0.5) {
          supportType = 'Emotional Supporter';
        }

        stats['supportType'] = supportType as int;
        patterns[entry.key] = stats;
      }
    } catch (e) {
      debugPrint("Error analyzing support patterns: $e");
    }
  ...
  Map<String, dynamic> _analyzeReciprocity(
    ...
    for (int i = 1; i < messages.length; i++) {
      ...
      final originalSenderName =
          userIdToName[previousMsg.senderId] ?? 'Unknown';
    ...
    for (final responder in userResponses.keys) {
      ...
      final totalResponses =
          responses.values.fold(0, (sum, count) => sum + count);
  ...
  Map<String, dynamic> _analyzeConversationBalance(
    ...
    for (int i = 0; i < messages.length; i++) {
      if (currentConv.isEmpty) {
        ...
        final timeDiff =
            messages[i].timestamp.difference(currentConv.last.timestamp);
    ...
    for (final conv in conversations) {
      ...
      final counts = userMessageCounts.values.toList()
    ...
    final totalAnalyzedConversations =
        balancedConversations + oneSidedConversations;
  ...
  Map<String, dynamic> _analyzeResponsePatterns(
    ...
    for (int i = 1; i < messages.length; i++) {
      ...
      final responseTime =
          currentMsg.timestamp.difference(previousMsg.timestamp);
      ...
      if (responseTime.inHours <= 24) {
        ...
        // Might be ignored
        userIgnored[originalSenderName] =
            (userIgnored[originalSenderName] ?? 0) + 1;
      }
    }
    ...
    for (final userName in userIdToName.values) {
      ...
      final avgSeconds =
          responseTimes.map((d) => d.inSeconds).reduce((a, b) => a + b) /
      ...
      responseProfiles[userName] = {
        ...
        'fastestResponse':
            _formatDuration(responseTimes.reduce((a, b) => a < b ? a : b)),
        'slowestResponse':
            _formatDuration(responseTimes.reduce((a, b) => a > b ? a : b)),
      };
    }
  ...
  Map<String, dynamic> _analyzeTopicControl(
    ...
    for (int i = 0; i < messages.length; i++) {
      ...
      if (topicWords.any(
          (word) => content.startsWith(word) || content.contains(' $word '))) {
      ...
      if (i == 0 ||
          (i > 0 &&
              message.timestamp.difference(messages[i - 1].timestamp).inHours >
                  2)) {
        conversationStarters[userName] =
            (conversationStarters[userName] ?? 0) + 1;
      }
    }
    ...
    for (final userName in userIdToName.values) {
      ...
      final controlScore =
          shifts + (starts * 2); // Weight conversation starts more
  ...
  Map<String, dynamic> _analyzeEmotionalDynamics(
    ...
    for (final message in messages) {
      ...
      if (positiveWords.any((word) => content.contains(word))) {
        emotionalPatterns[userName]!['positive'] =
            emotionalPatterns[userName]!['positive']! + 1;
        hasEmotion = true;
      }

      if (negativeWords.any((word) => content.contains(word))) {
        emotionalPatterns[userName]!['negative'] =
            emotionalPatterns[userName]!['negative']! + 1;
        hasEmotion = true;
      }

      if (supportiveWords.any((word) => content.contains(word))) {
        emotionalPatterns[userName]!['supportive'] =
            emotionalPatterns[userName]!['supportive']! + 1;
        hasEmotion = true;
      }

      if (!hasEmotion) {
        emotionalPatterns[userName]!['neutral'] =
            emotionalPatterns[userName]!['neutral']! + 1;
      }
    }
  ...
  double _calculateRelationshipHealth(Map<String, dynamic> reciprocity,
      Map<String, dynamic> balance, Map<String, dynamic> responses) {
    ...
    final balancePercentage =
        double.tryParse(balance['balancePercentage'] ?? '0') ?? 0.0;
...
class ContentIntelligenceAnalyzer {
  ...
  Map<String, dynamic> _analyzeQuestionStatementPatterns(
    ...
    for (final message in messages) {
      ...
      if (content.endsWith('?') || content.contains('?')) {
        userPatterns[userName]!['questions'] =
            userPatterns[userName]!['questions']! + 1;
      } else if (content.endsWith('!') || content.contains('!')) {
        userPatterns[userName]!['exclamations'] =
            userPatterns[userName]!['exclamations']! + 1;
      } else if (questionStarters
          .any((starter) => content.startsWith(starter))) {
        // Implicit questions
        userPatterns[userName]!['questions'] =
            userPatterns[userName]!['questions']! + 1;
      } else if (_isCommand(content)) {
        userPatterns[userName]!['commands'] =
            userPatterns[userName]!['commands']! + 1;
      } else {
        userPatterns[userName]!['statements'] =
            userPatterns[userName]!['statements']! + 1;
      }
    }
  ...
  Map<String, dynamic> _analyzeCommunicationStyles(
    ...
    for (final message in messages) {
      ...
      // Count punctuation
      style['exclamationCount'] =
          (style['exclamationCount'] as int) + (content.split('!').length - 1);
      style['ellipsisCount'] =
          (style['ellipsisCount'] as int) + (content.split('...').length - 1);

      // Count emojis (basic pattern)
      final emojiPattern = RegExp(
          r'[\u{1F600}-\u{1F64F}|\u{1F300}-\u{1F5FF}|\u{1F680}-\u{1F6FF}|\u{2600}-\u{26FF}|\u{2700}-\u{27BF}]',
          unicode: true);
      style['emojiCount'] = (style['emojiCount'] as int) +
          emojiPattern.allMatches(content).length;
    ...
    for (final entry in userStyles.entries) {
      ...
      final capsPercentage = totalChars > 0
          ? ((style['capsCount'] as int) / totalChars) * 100
          : 0.0;
      final emojiPerMessage = (style['emojiCount'] as int) / totalMessages;
      final exclamationPerMessage =
          (style['exclamationCount'] as int) / totalMessages;
      ...
      styleProfiles[userName] = {
        ...
        'longMessagePercentage':
            ((style['longMessages'] as int) / totalMessages * 100)
                .toStringAsFixed(1),
        'shortMessagePercentage':
            ((style['shortMessages'] as int) / totalMessages * 100)
                .toStringAsFixed(1),
      };
    }
  ...
  Map<String, dynamic> _analyzeLanguagePatterns(
    ...
    for (final message in messages) {
      ...
      userPatterns[userName]!['totalMessages'] =
          userPatterns[userName]!['totalMessages']! + 1;

      // Check for formal language
      if (formalWords.any((word) => content.contains(word))) {
        userPatterns[userName]!['formal'] =
            userPatterns[userName]!['formal']! + 1;
      }

      // Check for casual language
      if (casualWords.any((word) => content.contains(word))) {
        userPatterns[userName]!['casual'] =
            userPatterns[userName]!['casual']! + 1;
      }

      // Check for filler words
      if (fillerWords.any((word) => content.contains(word))) {
        userPatterns[userName]!['filler'] =
            userPatterns[userName]!['filler']! + 1;
      }

      // Check for intensifiers
      if (intensifiers.any((word) => content.contains(word))) {
        userPatterns[userName]!['intensifier'] =
            userPatterns[userName]!['intensifier']! + 1;
      }
    }
    ...
    for (final entry in userPatterns.entries) {
      ...
      final intensifierPercent =
          (patterns['intensifier']! / totalMessages) * 100;
  ...
  Map<String, dynamic> _analyzeInformationSharing(
    ...
    for (final message in messages) {
      ...
      userSharing[userName]!['totalMessages'] =
          userSharing[userName]!['totalMessages']! + 1;
      ...
      if (RegExp(r'\b\d{3,}\b').hasMatch(content)) {
        userSharing[userName]!['numbers'] =
            userSharing[userName]!['numbers']! + 1;
      }
      ...
      if (locationWords.any((word) => content.toLowerCase().contains(word))) {
        userSharing[userName]!['locations'] =
            userSharing[userName]!['locations']! + 1;
      }
    }
    ...
    for (final entry in userSharing.entries) {
      ...
      final totalInfoShared = sharing['links']! +
          sharing['numbers']! +
          sharing['dates']! +
          sharing['locations']!;
      ...
      } else if (sharing['links']! > sharing['numbers']! &&
          sharing['links']! > 3) {
  ...
  Map<String, dynamic> _analyzeConversationThreads(
    ...
    for (int i = 1; i < messages.length; i++) {
      ...
      userThreadBehavior.putIfAbsent(
          currentUser,
          () => {
  ...
  Map<String, dynamic> _calculateIntelligenceScores(
    ...
    for (final userName in allUsers) {
      ...
      if (vocabulary.containsKey(userName)) {
        final vocabScore =
            double.tryParse(vocabulary[userName]['complexityScore'] ?? '50') ??
                50.0;
      ...
      if (questionStatement.containsKey(userName)) {
        final questionPercent = double.tryParse(
                questionStatement[userName]['questionPercentage'] ?? '0') ??
            0.0;
        if (questionPercent > 20) totalScore += 15; // Bonus for being curious
        if (questionPercent > 35)
          totalScore += 10; // Extra bonus for high curiosity
      }

      // Information sharing contribution (30% weight)
      if (infoSharing.containsKey(userName)) {
        final infoRate =
            double.tryParse(infoSharing[userName]['infoSharingRate'] ?? '0') ??
                0.0;
        if (infoRate > 15) totalScore += 10; // Bonus for sharing information
        if (infoRate > 25)
          totalScore += 10; // Extra bonus for high info sharing
      }
...
class TemporalInsightAnalyzer {
  ...
  Map<String, dynamic> _analyzeResponseTimeEvolution(
    ...
    final timeSpan =
        messages.last.timestamp.difference(messages.first.timestamp);
    final segmentDays =
        timeSpan.inDays > 365 ? 30 : 7; // Monthly or weekly segments
    ...
    for (int i = 1; i < messages.length; i++) {
      ...
      final responseTime =
          currentMsg.timestamp.difference(previousMsg.timestamp);
      if (responseTime.inHours > 24)
        continue; // Skip if too long to be a response

      final responderName = userIdToName[currentMsg.senderId] ?? 'Unknown';
      final segmentStart = messages.first.timestamp;
      final daysSinceStart =
          currentMsg.timestamp.difference(segmentStart).inDays;
    ...
    for (final entry in userResponseTrends.entries) {
      ...
      final firstHalf =
          avgResponseTimes.take(avgResponseTimes.length ~/ 2).toList();
      final secondHalf =
          avgResponseTimes.skip(avgResponseTimes.length ~/ 2).toList();
  ...
  Map<String, dynamic> _analyzeIntensityWaves(
    ...
    for (final message in messages) {
      ...
      userDailyCounts[userName]![dateKey] =
          (userDailyCounts[userName]![dateKey] ?? 0) + 1;
    }

    // Find intensity waves (periods of high/low activity)
    final sortedDates = dailyMessageCounts.keys.toList()..sort();
    final dailyCounts =
        sortedDates.map((date) => dailyMessageCounts[date]!).toList();
    ...
    for (int i = 1; i < rollingAverages.length - 1; i++) {
      ...
      if (current > previous &&
          current > next &&
          current >
              rollingAverages.reduce((a, b) => a + b) /
    ...
    for (final userName in userIdToName.values) {
      ...
      final avgPeakActivity =
          totalPeakDays > 0 ? peakActivity / totalPeakDays : 0.0;
      final avgValleyActivity =
          totalValleyDays > 0 ? valleyActivity / totalValleyDays : 0.0;
      ...
      userIntensityPatterns[userName] = {
        ...
        'peakToValleyRatio': avgValleyActivity > 0
            ? (avgPeakActivity / avgValleyActivity).toStringAsFixed(1)
            : 'N/A',
      };
    }
  ...
  Map<String, dynamic> _analyzeRelationshipEvolution(
    ...
    for (int quarter = 0; quarter < 4; quarter++) {
      final startTime = messages.first.timestamp
          .add(Duration(days: quarter * quarterDuration.inDays));
      final endTime = quarter == 3
          ? messages.last.timestamp
          : messages.first.timestamp
              .add(Duration(days: (quarter + 1) * quarterDuration.inDays));

      final quarterMessages = messages
          .where((msg) =>
              msg.timestamp.isAfter(startTime) &&
              msg.timestamp.isBefore(endTime))
          .toList();
      ...
      for (int i = 0; i < quarterMessages.length; i++) {
        ...
        if (supportWords
            .any((word) => msg.content.toLowerCase().contains(word))) {
        ...
        if (i > 0 && quarterMessages[i - 1].senderId != msg.senderId) {
          final responseTime =
              msg.timestamp.difference(quarterMessages[i - 1].timestamp);
      ...
      final avgMessagesPerUser =
          totalUsers > 0 ? totalMessages / totalUsers : 0.0;

      // Calculate engagement score (questions + support + response speed)
      double engagementScore = 50.0;

      final totalQuestions =
          userQuestions.values.fold(0, (sum, count) => sum + count);
      final totalSupport =
          userSupport.values.fold(0, (sum, count) => sum + count);

      engagementScore +=
          (totalQuestions / totalMessages) * 30; // Question engagement
      engagementScore +=
          (totalSupport / totalMessages) * 20; // Emotional engagement

      // Response time factor
      final allResponseTimes =
          userResponseTimes.values.expand((times) => times).toList();
      ...
      quarterlyAnalysis.add({
        ...
        'avgResponseTimeSeconds': allResponseTimes.isNotEmpty
            ? (allResponseTimes.reduce((a, b) => a + b) /
    ...
    for (final message in messages) {
      ...
      for (final topic in topicKeywords.keys) {
        ...
        if (keywords.any((keyword) => content.contains(keyword))) {
          monthlyTopics[monthKey]![topic] =
              (monthlyTopics[monthKey]![topic] ?? 0) + 1;
  ...
  Map<String, dynamic> _analyzePatternChanges(
    ...
    final recentMessages =
        messages.skip(messages.length - quarterSize).toList();

    final earlyPatterns = _analyzeQuarterPatterns(earlyMessages, userIdToName);
    final recentPatterns =
        _analyzeQuarterPatterns(recentMessages, userIdToName);

    final Map<String, Map<String, dynamic>> patternChanges = {};

    for (final userName in userIdToName.values) {
      if (!earlyPatterns.containsKey(userName) ||
          !recentPatterns.containsKey(userName)) continue;
  ...
  Map<String, Map<String, double>> _analyzeQuarterPatterns(
    ...
    for (final message in messages) {
      ...
      patterns[userName]!['messageCount'] =
          patterns[userName]!['messageCount']! + 1;
      patterns[userName]!['avgLength'] =
          patterns[userName]!['avgLength']! + content.length;

      if (content.contains('?')) {
        patterns[userName]!['questionRate'] =
            patterns[userName]!['questionRate']! + 1;
      }

      final emojiCount = RegExp(r'[\u{1F600}-\u{1F64F}]', unicode: true)
          .allMatches(content)
          .length;
      patterns[userName]!['emojiRate'] =
          patterns[userName]!['emojiRate']! + emojiCount;
    }
  ...
  Map<String, dynamic> _analyzeActivityCorrelation(
    ...
    for (final message in messages) {
      ...
      userHourlyActivity[userName]![hour] =
          (userHourlyActivity[userName]![hour] ?? 0) + 1;
    }
    ...
    return {
      ...
      'highestCorrelation': sortedCorrelations.isNotEmpty
          ? sortedCorrelations.first.value.toStringAsFixed(2)
          : '0.00',
      'bestSyncedPair':
          sortedCorrelations.isNotEmpty ? sortedCorrelations.first.key : 'None',
    };
  }

  double _calculateHourlyCorrelation(
      Map<int, int> user1Activity, Map<int, int> user2Activity) {
    ...
    final pSum = List.generate(
            user1Values.length, (i) => user1Values[i] * user2Values[i])
        .reduce((a, b) => a + b);

    final num = pSum - (sum1 * sum2 / n);
    final den =
        math.sqrt((sum1Sq - sum1 * sum1 / n) * (sum2Sq - sum2 * sum2 / n));
  ...
  Map<String, dynamic> _calculateOverallTrends(
    ...
    if (trends
        .any((t) => t.contains('faster') || t.contains('Growing Closer'))) {
      overallEvolution = 'Positive Evolution 📈';
    } else if (trends
        .any((t) => t.contains('slower') || t.contains('Growing Apart'))) {
      overallEvolution = 'Concerning Changes 📉';
    } else if (trends
        .any((t) => t.contains('stable') || t.contains('Stable'))) {
  ...
  Map<String, dynamic> _insufficientDataResults(int days) {
    return {
      'temporalInsights': {
        'message':
            'Insufficient data for temporal analysis (only $days days). Need at least 30 days.',
        'timeSpanDays': days,
